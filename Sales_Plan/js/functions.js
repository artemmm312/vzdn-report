	...
	await $.ajax({
		type: 'POST', url: 'src/handler.php', success: function (response) {
			...
			if ($type_of_plane.val() === 'Общий') {
				...
				if(isNaN(+pct_quantity_of_overall) || +pct_quantity_of_overall === Infinity) pct_quantity_of_overall = 0.00;

				let pct_quantity_of_tare = ((100 * quantity_of_tare) / plane_quantity_of_tare).toFixed(2);
				if(isNaN(+pct_quantity_of_tare) || +pct_quantity_of_tare === Infinity) pct_quantity_of_tare = 0.00;

				let pct_quantity_of_drink = ((100 * quantity_of_drink) / plane_quantity_of_drink).toFixed(2);
				if(isNaN(+pct_quantity_of_drink) || +pct_quantity_of_drink === Infinity) pct_quantity_of_drink = 0.00;
            ...
            <div class="col-11 text-center">
              ...
              <div class="progress shadow">
				...
				for (let user_setting of users_plane) {
					...
					if(isNaN(+pct_overall_product) || +pct_overall_product === Infinity) pct_overall_product = 0.00;

					let pct_drink_product = ((100 * drink_product) / plane_of_drink_product).toFixed(2);
					if(isNaN(+pct_drink_product) || +pct_drink_product === Infinity) pct_drink_product = 0.00;

					let pct_tare_product = ((100 * tare_product) / plane_of_tare_product).toFixed(2);
					if(isNaN(+pct_tare_product) || +pct_tare_product === Infinity) pct_tare_product = 0.00;
